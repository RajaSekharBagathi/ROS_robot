...
#change function behavious like switch for when to change from followwall to gotopoint
def change_state(state):
    ...
    rospy.loginfo(log)
    if state_ == 0:
        resp = srv_client_go_to_point_(True,'go_to_point')
        resp = srv_client_wall_follower_(False,'wall_follower')
    if state_ == 1:
        resp = srv_client_go_to_point_(False,'go_to_point')
        resp = srv_client_wall_follower_(True,'wall_follower')
    if state_ == 2:
        resp = srv_client_go_to_point_(False,'go_to_point')
        resp = srv_client_wall_follower_(True,'wall_follower')


#to calculate the distance b/w two points we use math formula sqrt(a2 + b2)
